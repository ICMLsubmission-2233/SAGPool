...
class SAGPool(torch.nn.Module):
    ...
    def __init__(self, in_channels, ratio=0.5):
        ...
        self.conv = GraphConv(in_channels,1)
        self.add_module('attention_layer',self.conv)
    def forward(self, x, edge_index, two_hop, edge_attr=None, batch=None):
        """"""
        if batch is None:
            batch = edge_index.new_zeros(x.size(0))

        #sp_A = torch.sparse.FloatTensor(edge_index, torch.ones(edge_index.shape[1]),torch.Size([edge_index.shape[1],edge_index.shape[1]])) 
        #ds_A = sp_A.to_dense() 
        #hop_edge_index = (ds_A + sp_A.matmul(ds_A)).to_sparse()._indices()

        score = self.conv(x,two_hop).view(-1)
        ...
        two_hop, _ = filter_adj(
            two_hop, edge_attr, perm, num_nodes=score.size(0))

        return x, edge_index, two_hop, edge_attr, batch, perm
