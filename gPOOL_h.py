...
parser.add_argument('--dataname', type=str, default='DD',
                    help='DD/PROTEINS/NCI1/NCI109/Mutagenicity/Letter-high(med,low)/TWITTER-Real-Graph-Partial/COIL-DEL/COIL-RAG/AIDS')
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.pool1 = TopKPooling(nhid, ratio=args.ratio)
        self.conv2 = GraphConv(nhid, nhid)
        self.pool2 = TopKPooling(nhid, ratio=args.ratio)
        self.conv3 = GraphConv(nhid, nhid)
        self.pool3 = TopKPooling(nhid, ratio=args.ratio)
...
f = open(os.path.join('./data',args.dataname,args.mode+'_topk_result.csv'), 'a', encoding='utf-8', newline='')
wr = csv.writer(f)
