...
def global_sort_pool(x, batch, k):
    ...
    if N >= k:
        ...
        expand_batch_x = torch.full(size=(B, k, D), fill_value=fill_value).to(perm.device)
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        #self.pool = global_sort_pool(nhid*3, min_nodes)
        
        #self.oneD_conv = torch.nn.Conv1d(1,1,nhid,nhid)

        self.lin1 = torch.nn.Linear(nhid*3*2, nhid)
    ...
    def forward(self, data):
        ...
        x = global_sort_pool(x, batch, min_nodes)
        #x = self.oneD_conv(x.unsqueeze(1)).squeeze()
        x_sum = torch.sum(x,dim=1)
        ind = torch.sum((torch.sum(x,dim=2,keepdim=True) != 0),dim=1)
        
        
        x_sum = x_sum/ind.type(torch.FloatTensor).to(device)
        #print(x.min())
        #x[x == 0] = x.min()-1
        ...
        x = torch.cat([x_sum, torch.max(x,dim=1)[0]], dim=1)
        #print(x.shape)
...
f = open(os.path.join('./data',args.dataname,args.mode+'_sortpool_global_REAL_FINAL_result.csv'), 'a', encoding='utf-8', newline='')
wr = csv.writer(f)
