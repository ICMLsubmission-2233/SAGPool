...
class SAGPool(torch.nn.Module):
    ...
    def __init__(self, in_channels, ratio=0.5):
        ...
        self.conv_1 = GraphConv(in_channels,int(in_channels/2))
        self.leakyrelu = torch.nn.LeakyReLU(0.1)
        self.conv_2 = GraphConv(int(in_channels/2),1)
        self.add_module('attention_layer1',self.conv_1)
        self.add_module('attention_layer2',self.conv_2)
    def forward(self, x, edge_index, edge_attr=None, batch=None):
        """"""
        if batch is None:
            batch = edge_index.new_zeros(x.size(0))

        score = self.conv_1(x,edge_index)
        score = self.conv_2(self.leakyrelu(score),edge_index).view(-1)
